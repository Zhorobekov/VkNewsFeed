...
//  VkNewsFeed
//
//  Created by Эрмек Жоробеков on 20.05.2022.
//
...
class RowLayout: UICollectionViewLayout {
    ...
    static func rowHeightCounter(superViewWidth: CGFloat, photosArray: [CGSize]) -> CGFloat? {
        ...
        let photoWidthMinRation = photosArray.min { (first, second) -> Bool in
            (first.height / first.width) < (second.height / second.width)
        }
        
        guard let myPhotoWidthMinRation = photoWidthMinRation else { return nil }
